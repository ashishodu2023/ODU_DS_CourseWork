...
import random
from BoardConfig import BoardConfig as config

# using numpy, create an empty matrix of 6 rows and 7 columns
def create_board():
    board = np.zeros((config.ROWS, config.COLS))
    return board
...
def draw_board(board):
    for c in range(config.COLS):
        for r in range(config.ROWS):
            ...
            if board[r][c] == 0:
                pygame.draw.circle(screen, config.BLACK, (int(c * SQUARESIZE + SQUARESIZE/2), int(r* SQUARESIZE + SQUARESIZE + SQUARESIZE/2)), circle_radius)
            elif board[r][c] == 1:
                pygame.draw.circle(screen, config.RED, (int(c * SQUARESIZE + SQUARESIZE/2), int(r* SQUARESIZE + SQUARESIZE + SQUARESIZE/2)), circle_radius)
            else :
                pygame.draw.circle(screen, config.YELLOW, (int(c * SQUARESIZE + SQUARESIZE/2), int(r* SQUARESIZE + SQUARESIZE + SQUARESIZE/2)), circle_radius)
...
def score_position(board, piece):
    ...
    center_array = [int(i) for i in list(board[:,config.COLS//2])]
...
def is_terminal_node(board):
    return winning_move(board, config.PLAYER_PIECE) or winning_move(board, config.AI_PIECE) or len(get_valid_locations(board)) == 0
...
def minimax(board, depth, alpha, beta, maximizing_player):
    ...
    if depth == 0 or is_terminal:
        if is_terminal: # winning move 
            if winning_move(board, config.AI_PIECE):
                return (None, 10000000)
            elif winning_move(board, config.PLAYER_PIECE):
                return (None, -10000000)
            else:
                return (None, 0)
        # if depth is zero, we simply score the current board
        else: # depth is zero
            return (None, score_position(board, config.AI_PIECE))
    ...
    if maximizing_player:
        ...
        for col in valid_locations:
            ...
            b_copy = board.copy()
            drop_piece(b_copy, row, col, config.AI_PIECE)
            # recursive call
    ...
    # same as above, but for the minimizing player
        ...
        for col in valid_locations:
            row = get_next_open_row(board, col)
            b_copy = board.copy()
            drop_piece(b_copy, row, col, config.PLAYER_PIECE)
...
def get_valid_locations(board):
    ...
    for column in range(config.COLS):
...
turn = random.randint(config.PLAYER_TURN, config.AI_TURN)
...
while not game_over:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION and not_over:
            pygame.draw.rect(screen, config.BLACK, (0, 0, width, SQUARESIZE))
            xpos = pygame.mouse.get_pos()[0]
            if turn == config.PLAYER_TURN:
                pygame.draw.circle(screen, config.RED, (xpos, int(SQUARESIZE/2)), circle_radius )
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and not_over:
            ...
            if turn == config.PLAYER_TURN:
                ...
                if is_valid_location(board, col):
                    row = get_next_open_row(board, col)
                    drop_piece(board, row, col, config.PLAYER_PIECE)
                    if winning_move(board, config.PLAYER_PIECE):
                        print("PLAYER 1 WINS!")
    ...
    if turn == config.AI_TURN and not game_over and not_over:
        ...
        if is_valid_location(board, col):
            ...
            row = get_next_open_row(board, col)
            drop_piece(board, row, col, config.AI_PIECE)
            if winning_move(board, config.AI_PIECE):
                print("AI  WINS!")
                label = my_font.render("AI WINS!", 1, config.YELLOW)
                screen.blit(label, (40, 10))
                not_over = False
                t = Timer(3.0, end_game)
                t.start()
